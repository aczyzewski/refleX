...
class LogProgress(Callback):
    def __init__(self, logger_name, metrics, dec=6, step=1):
        super().__init__()
        self.metrics = ['epoch', 'val_loss'] + [metric.__name__ for metric in metrics]
        self.logger = logging.getLogger(logger_name)
        self.logger.info(', '.join(self.metrics))
        self.dec = dec
        self.step = step
        self.epoch = 0
    ...
    def on_epoch_end(self, metrics):
        ...
        if not (self.epoch % self.step):
            if type(metrics[0]) is np.ndarray:
                metrics = np.append([metrics[0][0]], metrics[1:])

            metrics = ', '.join(list(map(lambda x: str(np.round(x, self.dec)), metrics)))
            self.logger.info(f"{self.epoch}, {metrics}")
        
        self.epoch += 1

class EarlyStopping(Callback):
    def __init__(self, logger_name, patience=16):
        super().__init__()
        if logger_name:
            self.logger = logging.getLogger(logger_name)
    ...
    def on_epoch_end(self, metrics):
        ...
        if self.num_epochs_no_improvement > self.patience:
            if self.logger:
                self.logger.info("*** Earlystopping! ***")
            return True
